...
def _client() -> "OpenAI":
    # 强制清理环境变量中的代理设置，防止 older openai 库版本冲突
    ...
    if not api_key:
        # 尝试从数据库读取配置
        try:
            from core.db_tool import get_sys_configs
            configs = get_sys_configs()
            api_key = configs.get("DEEPSEEK_API_KEY", "").strip()
        except Exception:
            pass
            
    if not api_key:
        raise RuntimeError("未检测到 DEEPSEEK_API_KEY。请在服务器配置环境变量，或登录后台在'系统设置'中填入。")
