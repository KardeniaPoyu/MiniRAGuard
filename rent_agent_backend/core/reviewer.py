...
⚠️ 【严格约束】：
在输出的 `legal_basis` 字段中，**严禁自行编造**或引用上述【参考法条】中未包含的任何具体或通用法律条文。
由于民法基本原则（如第三、四条等）通常不能直接作为合同条款违约或不合理的具体判决依据，如果【参考法条】中没有对应的具体条文，请将 `legal_basis` 填写为“缺乏对应具体条文支持”或“涉及不合理格式条款”，重点在 `reason` 说明其不公平性。
